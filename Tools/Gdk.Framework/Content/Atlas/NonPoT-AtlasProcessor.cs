...
namespace Gdk.Content
{
    public class AtlasProcessor : Processor
    {
        ...
        // The maximum sized texture that the atlas's will attempt to fit images into
        ...
        private void ProcessSheet(Atlas atlas, XmlNode sheetNode, int sheetId)
        {
            ...
            // Find the biggest dimension of the images
            int largestDimension = 0;
            foreach (AtlasImage image in sheet.Images)
            {
                if (image.Texture.Width > largestDimension)
                    largestDimension = image.Texture.Width;
                if (image.Texture.Height > largestDimension)
                    largestDimension = image.Texture.Height;
            }
            ...
            int highestNumImagesFit = 0;

            // Loop through some test sheet sizes, from the Biggest image size, to the max texture size
            for (float xSizeT = 0; xSizeT <= 1.0f; xSizeT += 0.1f)
            {
                int attemptSizeX = (int)MathUtility.Lerp(xSizeT, largestDimension, MAX_TEXTURE_SIZE);

                for (float ySizeT = 0; ySizeT <= 1.0f; ySizeT += 0.1f)
                {
                    int attemptSizeY = (int)MathUtility.Lerp(ySizeT, largestDimension, MAX_TEXTURE_SIZE);
                    ...
                    // Do the fit attempt
                    if (FitImages(fitAttempt, sheet.Images) == true)
                        fitAttempts.Add(fitAttempt);
                    if (fitAttempt.OutImagesFit > highestNumImagesFit)
                        highestNumImagesFit = fitAttempt.OutImagesFit;
            ...
            // Did we not find any fits?
            if (fitAttempts.Count == 0)
            {
                Context.LogError("Unable to fit all the images onto a single sheet.");
                Context.LogError("- Consider splitting the images onto multiple sheets.");
                Context.LogError("- Best Approximate fit (" + highestNumImagesFit + " of " + sheet.Images.Count + " images)");
                Context.LogError("- Max sheet size: " + MAX_TEXTURE_SIZE);

                throw new BuildException("Unable to fit all images onto a single sheet.");
            }
            ...
            {
                // Does this fit make better use of space?
                if (fitAttempt.OutSheetSpaceUsage > bestFit.OutSheetSpaceUsage)
                {
                    bestFit = fitAttempt;
                    continue;
                }

                // Does this fit have a shorter diagonal?
            ...
            // Log the sheet size
            Context.LogVerbose("  +-- Best Fit Found. Sheet Size: " + bestFit.OutWidth + "x" + bestFit.OutHeight);

            // If the best fit is still low, lot a warning
            int sheetUsagePercentage = (int)(bestFit.OutSheetSpaceUsage * 100);
            if (sheetUsagePercentage < 75)
            {
                Context.LogWarn("  +-- Warning: only " + sheetUsagePercentage + "% of the sheet texture is used, consider resizing the images or splitting into multiple sheets.");
            }
            else
            {
                Context.LogVerbose("  +-- Sheet usage: " + sheetUsagePercentage + "%");
            }

            // Refit all the images using the best fit
            bestFit.InWidth = bestFit.OutWidth;
            bestFit.InHeight = bestFit.OutHeight;
            FitImages(bestFit, sheet.Images);
        ...
        /// </summary>
        private bool FitImages(AtlasSheetFitInfo fitInfo, List<AtlasImage> images)
        {
