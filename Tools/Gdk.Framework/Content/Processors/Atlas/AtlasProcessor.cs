...
namespace Gdk.Framework.Content
{
    ...
    public class AtlasProcessor : Processor
    {
        ...
        public override void Process()
        {
            ...
            Context.Log("Processing Atlas: " + atlasFilePath);
            ...
            Context.Log("- Loading Animations");
        ...
        private void BuildSheet(Atlas atlas, PixelFormats pixelFormat, List<AtlasImage> images)
        {
            ...
            Context.Log("+-- Building Sheet");
            Context.Log("  +-- Finding best fit sheet size");
            ...
            Context.Log("  +-- Best Fit Found. Sheet Size(" + bestFit.OutWidth + "x" + bestFit.OutHeight + ") using (" + bestFit.OutImagesFit + " of " + images.Count + " images)");
            ...
            {
                Context.GenerateWarning("Only " + sheetUsagePercentage + "% of the sheet texture is used, consider resizing the images, or tweaking the sheet fitting heuristics");
            }
            else
            {
                Context.Log("  +-- Sheet space usage: " + sheetUsagePercentage + "%");
            }
            ...
            Context.Log("  +-- Pixel Format: " + sheet.PixelFormat.ToString());
            ...
            foreach (AtlasImage image in sheet.Images)
            {
                ...
                Context.Log(string.Format("  +-- Image Size({0}x{1}) atlas to ({2},{3}) - [Image:{4}/{5}]", image.Texture.Width, image.Texture.Height, (int)image.SheetPosition.X, (int)image.SheetPosition.Y, currentImageId, sheet.Images.Count));
            }
        }
        ...
        private void ProcessImage(Atlas atlas, XmlNode imageNode, ImageProcessParameters defaultImageProcessParameters)
        {
			...
			pathFromXml = PathUtilities.FixPathSeperators(pathFromXml);
            ...
            if (numFiles == 0)
            {
                throw new BuildException("Image file(s) not found: " + PathUtilities.Combine(atlas.ContentFolder, pathFromXml));
            }

            // Dont allow the imageName field if we have more than 1 file
            if (numFiles > 1 && XmlUtilities.AttributeExists(imageNode, "Name") == true)
            {
                throw new BuildException("The Name attribute is NOT allowed on <Image> elements with a wildcard in the Path. [" + PathUtilities.Combine(atlas.ContentFolder, pathFromXml) + "]");
            }
            ...
            foreach (string imageFullPath in filePaths)
            {
				...
				imagePath = PathUtilities.FixPathSeperators(imagePath);
                ...
                Context.Log("  +-- Loading PNG: " + imageFullPath);
                ...
                if (imageProcessParameters.UseRefPointColor == false)
                {
                    ...
                    Context.Log("  +-- RefPoint(" + image.ReferencePoint.X + "," + image.ReferencePoint.Y + ")   [by number]");
                }
                else
                {
                    ...
                    Context.Log("  +-- RefPoint(" + image.ReferencePoint.X + "," + image.ReferencePoint.Y + ")   [by color]");
                }
                ...
                foreach (int[] childPointCoords in imageProcessParameters.ChildPoints)
                {
                    ...
                    Context.Log("  +-- ChildPoint(" + childPoint.X + "," + childPoint.Y + ")   [by number]");
                }
                ...
                foreach (Color childPointColor in imageProcessParameters.ChildPointColors)
                {
                    ...
                    if (childPoint.X >= 0)
                    {
                        ...
                        Context.Log("  +-- ChildPoint(" + childPoint.X + "," + childPoint.Y + ")   [by color]");
                    }
                    else
                    {
                        Context.GenerateWarning("Color (" + childPointColor.R + "," + childPointColor.G + "," + childPointColor.B + ") not found for child point in image: " + imageName);
                    }
                }
                ...
                if (imageProcessParameters.ColorKey != Color.TransparentBlack)
                {
                    ...
                    Context.Log(" +-- Replaced color key \"" + imageProcessParameters.ColorKey.R + "," + imageProcessParameters.ColorKey.G + "," + imageProcessParameters.ColorKey.B + ") with TransparentBlack");
                }
                ...
                if (imageScale.X < 1.0f || imageScale.Y < 1.0f)
                {
                    ...
                    // Log the scaling
                    Context.Log("  +-- Scaled Image from(" + imageTexture.Width + "," + imageTexture.Height + ") to (" + newWidth + "," + newHeight + ")");
                ...
                {
                    // Log the image size
                    Context.Log("  +-- Pixel Size(" + imageTexture.Width + "," + imageTexture.Height + ")");
                }
                ...
                if (foundNonTransparent == false)
                {
                    ...
                    Context.GenerateWarning("Image is completely transparent, skipping white space cropping: " + imageName);
                ...
                else
                {
                    ...
                    Context.Log("  +-- White space cropping offsets (" + xStart + ", " + yStart + ")-(" + xEnd + ", " + yEnd + ")");
        ...
        private void ProcessAnimation(Atlas atlas, XmlNode animationNode)
        {
            ...
            foreach (XmlNode frameNode in animationNode.SelectNodes("Frame"))
            {
				...
				imageName = PathUtilities.FixPathSeperators(imageName);
        ...
        private void Write(Atlas atlas)
        {
            ...
            string atlasFullPath = PathUtilities.Combine(Context.OutputFolder, atlasRelPath);
            ...
            foreach (AtlasSheet sheet in atlas.Sheets)
            {
                ...
                string sheetFullPath = PathUtilities.Combine(Context.OutputFolder, sheetRelPath);
