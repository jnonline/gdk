...
namespace Gdk.Framework.Content
{
    ...
    public class ShaderProcessor : Processor    
    {
        ...
        public override void Process()
        {
            ...
            Context.Log("Parsing Shader XML");
            ...
            Context.Log("Compiling Shader Programs");
            ...
            Context.Log("Writing Binary Shader");
        ...
        private void ParseShader()
        {
            ...
            Context.Log(" - Parsing RenderState");
            ...
            Context.Log(" - Parsing AttributeBindings");
        ...
        private void ParseTechnique(XmlNode xmlTechnique)
        {
            ...
            Context.Log(" +-- Parsing Technique: " + technique.Name);
        ...
        string RecurseProcessIncludes(string input, string parentFolder)
        {
            ...
            while ((index = text.IndexOf("#include")) >= 0)
            {
                ...
                Context.Log(" +-- Processing include: " + includeFilePath);
        ...
        void ParseParameters(XmlNode xmlParameters)
        {
            ...
            {
                Context.GenerateWarning("No <Parameters> element was found in the shader.. is this intentional?");
                return;
            }
        ...
        private void CompileShaders()
        {
            ...
            {
                Context.Log(" - Compiling Technique: " + technique.Name);
        ...
        private void CompileTechnique(ShaderTechnique technique)
        {
            ...
            try
            {
                ...
                Context.Log(" +-- Compiling Vertex Shader");
                vertexShaderId = CompileGLShader(technique.VertexShaderText, ShaderType.VertexShader);

                // Compile the fragment shader
                Context.Log(" +-- Compiling Fragment Shader");
                fragmentShaderId = CompileGLShader(technique.FragmentShaderText, ShaderType.FragmentShader);
                ...
                Context.Log(" +-- Linking Shader Program");
                ...
                Context.Log(" +-- Retrieving Attributes");
				
                // Get the # of attributes
                int numAttributes = 0;
				GL.GetProgram(programId, ProgramParameter.ActiveAttributes, out numAttributes);
                ...
                if (numAttributes > 0)
                {
					...
					GL.GetProgram(programId, ProgramParameter.ActiveAttributeMaxLength, out maxAttributeNameLength);
                    ...
                    for (int attrIndex = 0; attrIndex < numAttributes; attrIndex++)
                    {
						...
						GL.GetActiveAttrib(programId, attrIndex, maxAttributeNameLength, out len, out size, out type, nameBuilder);
                        ...
                        Context.Log(" +---- Attribute [Name: " + attrName + "] [Type: " + attrType + "] [Size: " + size + "]");
                    }
                }
                ...
                Context.Log(" +-- Retrieving Uniforms");

                // Get the # of uniforms
                int numUniforms = 0;
				GL.GetProgram(programId, ProgramParameter.ActiveUniforms, out numUniforms);
                ...
                if (numUniforms > 0)
                {
					...
					GL.GetProgram(programId, ProgramParameter.ActiveUniformMaxLength, out maxUniformNameLength);
                    ...
                    for (int uniformIndex = 0; uniformIndex < numUniforms; uniformIndex++)
                    {
						...
						GL.GetActiveUniform(programId, uniformIndex, maxUniformNameLength, out len, out size, out type, nameBuilder);
						
						ShaderUniformType uniformType = (ShaderUniformType)type;
                        string uniformName = nameBuilder.ToString();
						
						// Strip array index "[0]" suffixes off of array uniforms
                        ...
                        Context.Log(" +---- Uniform [Name: " + uniformName + "] [Type: " + uniformType + "] [Size: " + size + "]");
                    }
                }
        ...
        /// </summary>
        private int CompileGLShader(string shaderText, ShaderType shaderType)
        {
			// Hack to compile OpenGLES shaders on an OpenGL context
            ...
            int status;
			GL.GetShader(shaderId, OpenTK.Graphics.OpenGL.ShaderParameter.CompileStatus, out status);

            // Did the compile fail?
            if (status != (int)All.True)
            {
				// Get the shader compiler log
				string shaderCompileLog = GL.GetShaderInfoLog(shaderId);
					
                // Is there a log
                if (shaderCompileLog.Length > 0)
                {
                    // Write the log
                    Context.GenerateError(
                        "Failed to compile " + shaderType.ToString() + "\n" +
                        shaderCompileLog
                        );
                }
                else
                {
                    // No Log
                    Context.GenerateError("Failed to compile " + shaderType.ToString() + ": Generic Error (no log)");
                }
            ...
            Context.Log(" +---- Successfully compiled " + shaderType.ToString());
        ...
        private bool LinkGLProgram(int programId)
        {
            ...
            if (status != (int)All.True)
            {
				...
				string shaderLinkLog = GL.GetProgramInfoLog(programId);
                
                // Is there a log
                if (shaderLinkLog.Length > 0)
                {
                    // Write the log
                    Context.GenerateError(
                        "Failed to link the shader program.\n" +
                        StringUtilities.EncodeHtml(shaderLinkLog)
                        );
                }
                else
                {
                    // No Log
                    Context.GenerateError("Failed to link the shader program: Generic Error (no log)");
                }
            ...
            // Link status == success
            Context.Log(" +---- Successfully linked the shader program");
        ...
        private void VerifyAttributes()
        {
            ...
            Context.Log(" - Verifying Attributes");
            ...
            foreach (KeyValuePair<string, ShaderAttribute> globalAttributeByName in shader.Attributes)
            {
                ...
                foreach(ShaderTechnique technique in shader.Techniques)
                {
                    ...
                    {
                        Context.GenerateError("Attribute [" + globalAttribute.Name + "] is NOT defined in all the techniques");
                        failed = true;
                    ...
                    {
                        Context.GenerateError("Attribute [" + globalAttribute.Name + "] in the technique [" + technique.Name + "] does not match the Size & Type of the first technique");
                        failed = true;
            ...
            foreach(ShaderTechnique technique in shader.Techniques)
            {
                ...
                foreach (KeyValuePair<string, ShaderAttribute> attributeByName in technique.Attributes)
                {
                    ...
                    {
                        Context.GenerateError("Attribute [" + attributeByName.Key + "] in the technique [" + technique.Name + "] is NOT defined in all the techniques");
                        failed = true;
                    }
                }
            }
            ...
            Context.Log(" - Verifying Attribute Bindings");
            ...
            foreach (KeyValuePair<int, string> attributeBinding in this.AttributeBindings)
            {
                ...
                {
                    Context.GenerateError("Cannot find the attribute [" + attributeBinding.Value + "] in all of the technique vertex shaders");
                    failed = true;
                }
            ...
            foreach (KeyValuePair<string, ShaderAttribute> attributeByName in shader.Attributes)
            {
                ...
                {
                    Context.GenerateError("The attribute [" + attributeByName.Key + "] has no matching Attribute Binding specified");
                    failed = true;
                }
            }
        ...
        private void VerifyUniforms()
        {
            ...
            Context.Log(" - Verifying Unique Uniforms");
            ...
            foreach(ShaderTechnique technique in shader.Techniques)
            {
                ...
                foreach(KeyValuePair<string, ShaderUniform> uniformByName in technique.Uniforms)
                {
                    ...
                    if(UniqueUniforms.ContainsKey(uniform.Name) == true)
                    {
                        ...
                        {
                            Context.GenerateError("The Uniform [" + uniform.Name + "] has mis-matched types across multiple Techniques");
                            failed = true;
                        }
                        ...
                        {
                            Context.GenerateError("The Uniform [" + uniform.Name + "] has mis-matched array sizes across multiple Techniques");
                            failed = true;
                        }
                    }
                    else
        ...
        {
            Context.Log(" - Verifying Parameters");
            bool failed = false;
            ...
            foreach (KeyValuePair<string, ShaderUniform> uniformByName in UniqueUniforms)
            {
                ...
                if (shader.Parameters.ContainsKey(uniform.Name) == false)
                {
                    ...
                    Context.GenerateWarning(" +-- Uniform without a matching parameter detected [" + uniform.Name + "].  (consider adding this parameter to the Shader XML)");
                }
                else
                {
                    ...
                    if (parameter.HasInitialValue == true)
                    {
                        ...
                        switch (parameter.UniformType)
                        {
                            ...
                            default:
                                Context.GenerateError("Parameter [" + parameter.Name + "] has an unknown type, cannot parse");
                                failed = true;
                                break;
                        }
                    } // is binding
            ...
            foreach (KeyValuePair<string, ShaderParameter> parameter in shader.Parameters)
            {
                ...
                {
                    Context.GenerateWarning(" +-- Parameter [" + parameter.Key + "] has no matching uniforms and will be ignored.  (consider removing it from the Shader XML)");
        ...
        private bool ParseBoolValue(ShaderParameter parameter, int vectorSize)
        {
            ...
            {
                Context.GenerateError("The Parameter [" + parameter.Name + "] contains an invalid number of elements for it's given array size & type.  [Expected: " + totalElements + "] [Actual: " + valuesList.Count + "]");
                return true;
            }
