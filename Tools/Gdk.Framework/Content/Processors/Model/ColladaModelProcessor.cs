...
namespace Gdk.Framework.Content.Model
{
    ...
    // ===============================================================
    public class ColladaModelProcessor : Processor
    {
        ...
        public override void Process()
        {
            ...
            ColladaSourceFile = PathUtilities.Combine(Context.ContentFolder, Asset.Path);
            ColladaSourceFolder = Path.GetDirectoryName(ColladaSourceFile);

            Context.Log("\n<b>==== Parsing Collada File ====</b>");

            // Load the collada source
            Source = new Content.COLLADA.ColladaDocument(ColladaSourceFile);

            // Log some collada stats
            Context.Log(" - COLLADA Version: " + Source.Version);
            Context.Log(" - Geometries: " + Source.Geometries.Count);
            Context.Log(" - Effects: " + Source.Effects.Count);
            Context.Log(" - Images: " + Source.Images.Count);
            Context.Log(" - Materials: " + Source.Materials.Count);
            Context.Log(" - Animation Clips: " + Source.AnimationClips.Count);
            Context.Log(" - Animations: " + Source.Animations.Count);
            Context.Log(" - Controllers: " + Source.Controllers.Count);
            Context.Log(" - Nodes: " + Source.Nodes.Count);
            Context.Log(" - Visual Scenes: " + Source.VisualScenes.Count);
            Context.Log(" - Root Scene: " + Source.Scene.VisualSceneInstance.Url);
           
            // Build the Gdk Model DOM from the Collada DOM
            // --------------------------------------------

            Context.Log("\n<b>==== Generating Raw Model ====</b>");
            GenerateModel();

            Context.Log("\n<b>==== Optimizing Model ====</b>");
            OptimizeModel();

            Context.Log("\n<b>==== Finalizing Model ====</b>");
            FinalizeModel();

            // Process any dependent assets.  (IE: textures)
            // -----------------------------

            Context.Log("\n<b>==== Processing Dependent Assets ====</b>");
            ProcessDependencies();

            // Write the binary model
            // --------------------------

            Context.Log("\n<b>==== Writing Binary Model ====</b>");
            WriteModel();
        }
        ...
        public void GenerateModel()
        {
            ...
            Context.Log("<b> - Setting up root transforms</b>");
            ...
            switch (this.Source.Asset.UpAxis)
            {
                ...
                case COLLADA.UpAxisTypes.Z_UP:
                    Context.Log(" +-- Converting collada data from Z_UP to Y_UP");
                    rootTransform *= Matrix.CreateRotationX(3.0f * MathUtility.PiOver2);
                    break;

                case COLLADA.UpAxisTypes.Y_UP:
                    Context.Log(" +-- Collada data is already in Y_UP format");
                    break;
            }
            ...
            {
                Context.Log(" +-- Scaling collada data to match [1-Unit = 1-Meter].  Current scale [1-Unit = " + this.Source.Asset.UnitMeter + "-Meters]");
                rootTransform *= Matrix.CreateScale(this.Source.Asset.UnitMeter);
            }
            else
                Context.Log(" +-- Collada data is already in the default scale [1-Unit = 1-Meter].");
            ...
            Context.Log("\n<b> - Processing Animation Clips</b>");
        ...
        public void ProcessColladaAnimationClip(COLLADA.AnimationClip colladaAnimationClip)
        {
            ...
            Context.Log(" +- Animation Clip: " + animationClip.Name + " [Collada ID: " + colladaAnimationClip.Id + "]");
        ...
        public void ProcessColladaAnimation(COLLADA.Animation colladaAnimation, Animation parentAnimation, string indent)
        {
            ...
            Context.Log(" +-" + indent + " Animation: " + animation.Name + " [Collada ID: " + colladaAnimation.Id + "]");
        ...
        public void ProcessNodeAnimations(COLLADA.Animation colladaAnimation, Animation animation, string indent)
        {
            ...
            foreach (COLLADA.Channel channel in colladaAnimation.Channels)
            {
                ...
                for(int index=0; index < inputValues.Count; index++)
                {
                    ...
                    Matrix transformMatrix = new Gdk.Framework.Matrix(
                ...
                if(nodeAnimation.KeyFrames.Count > 0)
                {
                    ...
                    Context.Log(" +---" + indent + " Node [" + node.Name + "] [Keyframes: " + nodeAnimation.KeyFrames.Count + "]");
                }
            }
        }
        ...
        /// </summary>
        public void ProcessColladaNode(COLLADA.Node colladaNode, Node parentNode, Gdk.Framework.Matrix parentTransform, string logIndent)
        {
            Context.Log(" +-" + logIndent + " Node: " + colladaNode.Id);

            // Get the local & full transform for this node
            Gdk.Framework.Matrix localTransform = COLLADA.TransformBase.GetCombinedTransforms(colladaNode.Transforms);
            Gdk.Framework.Matrix absoluteTransform = localTransform * parentTransform;
        ...
        /// </summary>
        public void ProcessColladaInstanceNode(COLLADA.InstanceNode nodeInstance, Node parentNode, Gdk.Framework.Matrix parentTransform, string logIndent)
        {
            ...
            if(controller.Skin != null)
            {
                ...
                if (controllerInstance.BindMaterial != null)
                {
                    ...
                    foreach (COLLADA.InstanceMaterial materialInstance in controllerInstance.BindMaterial.MaterialInstances)
                    {
                        ...
                        {
                            Context.GenerateWarning("An [instance_controller] in the node id=\"" + node.Name + "\" contains a material binding for a material symbol \"" + materialInstance.Symbol + "\" that doesnt exist on the mesh.  (this is usually safe to ignore)");
                            continue;
                        }
                ...
                Context.Log(" +---" + logIndent + " Mesh Instance (Skinned): " + mesh.ColladaID);
        ...
        public void ProcessColladaInstanceGeometry(COLLADA.InstanceGeometry geometryInstance, Node node, string logIndent)
        {
            ...
            {
                Context.GenerateWarning("The node id=\"" + node.Name + "\" contains an instance of an unknown/unrecognized mesh \"" + meshId + "\"");
                return;
            }
            ...
            if (geometryInstance.BindMaterial != null)
            {
                ...
                foreach (COLLADA.InstanceMaterial materialInstance in geometryInstance.BindMaterial.MaterialInstances)
                {
                    ...
                    if (mesh.TrianglesByMaterialSymbol.ContainsKey(materialInstance.Symbol) == false)
                    {
                        Context.GenerateWarning("An [instance_geometry] in the node id=\"" + node.Name + "\" contains a material binding for a material symbol \"" + materialInstance.Symbol + "\" that doesnt exist on the mesh.  (this is usually safe to ignore)");
                        continue;
                    }
            ...
            Context.Log(" +---" + logIndent + " Mesh Instance: " + mesh.ColladaID);
        ...
        public void ProcessColladaController(COLLADA.Controller controller)
        {
            ...
            {
                Context.Log(" +- Skin: " + controller.Id);
                ...
                Context.Log(" +--- Source Mesh: " + sourceMesh.ColladaID);
                ...
                Context.Log(" +--- Joints: " + skinnedMesh.NumJoints);
                ...
                Context.Log(" +--- Joints Per Vertex: " + fixedBonesPerVertex);
                ...
                if(didWeightNormalization == true)
                    Context.Log(" +--- Joint Weights Normalized");
            ...
            if (controller.Type == COLLADA.ControllerType.Morph)
            {
                ...
                Context.GenerateWarning("Morph controller detected \"" + controller.Id + "\".  Skipping processing as morph controllers are unsupported");
            }
        }
        ...
        public void ProcessColladaGeometry(COLLADA.Geometry colladaGeometry)
        {
            ...
            {
                Context.Log(" +- Mesh: " + colladaGeometry.Id);
                ...
                {
                    Context.GenerateWarning("The mesh \"" + mesh.ColladaID + "\" contains no triangle data, skipping");
                    return;
                }
                ...
                if ((mesh.Flags & Content.Model.MeshFlags.VertexHasColor) > 0)
                    vertexFormat += "C4 ";
                if ((mesh.Flags & Content.Model.MeshFlags.VertexHasTexCoords) > 0)
                    vertexFormat += "TC2a ";
                if ((mesh.Flags & Content.Model.MeshFlags.VertexHasTexCoords2) > 0)
                    vertexFormat += "TC2b ";

                Context.Log(" +--- Vertex Format: " + vertexFormat);
                ...
                {
                    Context.Log(" +--- Triangle Soup:  [Material: " + trianglesByMaterial.Key + "] [#Triangles: " + trianglesByMaterial.Value.Count + "]");
                }
            ...
            {
                Context.GenerateWarning("Unrecognized geometry: " + colladaGeometry.Id + " [skipping]");
            }
        }
        ...
        List<Triangle> GetTriangleSetForMaterialSymbol(Mesh mesh, string materialSymbol)
        {
            ...
            {
                Context.GenerateWarning("The mesh[id=\"" + mesh.ColladaID + "\"] does not have a material symbol, defaulting to the \"GDK-Unassigned\" symbol");
        ...
        public Vertex BuildModelVertex(Dictionary<string, object> namedVertexData)
        {
            ...
            foreach (KeyValuePair<string, object> dataItem in namedVertexData)
            {
                switch (dataItem.Key)
                {
                    ...
                    // Unknown Vertex Data
                    default:
                        Context.GenerateWarning("Unprocessed Vertex Data Item: " + dataItem.Key);
                        break;
                }
            }
        ...
        {
            Context.Log(" +- Material: " + colladaMaterial.Id);
            ...
            {
                case Content.COLLADA.TechniqueType.Blinn:
                    {
                ...
                case Content.COLLADA.TechniqueType.Phong:
                    {
                ...
                case Content.COLLADA.TechniqueType.Lambert:
                    {
            ...
            if(string.IsNullOrEmpty(diffuseTextureRef) == false)
            {
                ...
                material.Diffuse = Gdk.Framework.Color.White;
        ...
        public void OptimizeModel()
        {
            ...
            if (removeUnusedMeshes)
            {
                ...
                Context.Log(" - Checking for unused meshes");
                ...
                foreach (string meshId in meshIds)
                {
                    ...
                    if (instanceFound == false)
                    {
                        ...
                        Context.Log(" +- Removing unused mesh: \"" + meshId + "\"");
                    }
                }
            ...
            {
                Context.Log(" - Checking for unused materials");
                ...
                foreach (string materialId in materialIds)
                {
                    ...
                    if (instanceFound == false)
                    {
                        ...
                        Context.Log(" +- Removing unused material: \"" + materialId + "\"");
                    }
                }
            }
            ...
            if (removeUnusedMaterials)
            {
                Context.Log(" - Checking for unused nodes");
                ...
                for(int nodeIndex = 0; nodeIndex < this.ResultModel.Nodes.Count; nodeIndex++)
                {
                    ...
                    if(node.InUse == false)
                    {
                        ...
                        nodeIndex--;
                        Context.Log(" +- Removing unused node: \"" + node.Name + "\"");
...
*/
        ...
        public void FinalizeModel()
        {
            ...
            Context.Log("<b> - Finalizing Meshes</b>");
            ...
            Context.Log("\n<b> - Final Model Details</b>");
            Context.Log(" +- Nodes: " + this.ResultModel.Nodes.Count);
            Context.Log(" +- Materials: " + this.ResultModel.Materials.Count);
            Context.Log(" +- Meshes: " + this.ResultModel.Meshes.Count);
            Context.Log(" +- Mesh Instances: " + this.ResultModel.MeshInstances.Count);
            Context.Log(" +- Animations: " + this.ResultModel.Animations.Count);
            Context.Log(" +- Animation Clips: " + this.ResultModel.AnimationClips.Count);
            ...
            if (this.ResultModel.MeshInstances.Count == 0)
                Context.GenerateWarning("No meshes were instanced in this model, so the model data is empty..  This is likely a bad thing...");
        }
        ...
        {
            Context.Log(" +-- Mesh: " + mesh.ColladaID);
            ...
            {
                Context.Log(" +---- Auto-generating normals");
            }
            ...
            if (originalVertices > 0)
                Context.Log(" +---- Vertices: [Original: " + originalVertices + "] [Reduced: " + reducedVertices + "] - " + (int)((1.0f - reducedVertices / (float)originalVertices) * 100) + "% reduction");
            Context.Log(" +---- Indices: " + mesh.Indices.Count);

            
            // Log the mesh part details
            foreach(MeshPart meshPart in mesh.MeshParts)
            {
                Context.Log(" +---- Mesh Part [Material: " + meshPart.MaterialSymbol + "] [IndexCount: " + meshPart.IndexCount + "] [IndexStart: " + meshPart.IndexStart + "]");
            }
        ...
        public void ProcessDependentTexture(ref string textureRelPath, PixelFormats pixelFormat)
        {
            ...
            string outputRelPath = PathUtilities.Combine(Path.GetDirectoryName(Asset.Path), textureRelPath);
            outputRelPath = Path.ChangeExtension(outputRelPath, "gdkimage");
            string outputFullPath = PathUtilities.Combine(Context.OutputFolder, outputRelPath);

            // Add the dependency on the input texture file
            string textureFullPath = PathUtilities.Combine(ColladaSourceFolder, textureRelPath);
        ...
        public void WriteModel()
        {
            ...
            string fullDestPath = PathUtilities.Combine(Context.OutputFolder, relativeDestPath);
